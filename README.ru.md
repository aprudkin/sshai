...
`Windows PowerShell` — по SSH. По умолчанию предпочитается `PowerShell 7` (`pwsh`), а если он
недоступен, используется `Windows PowerShell 5.1`; для отдельного запуска можно явно потребовать
любую из этих оболочек. Полученный вывод сохраняется локально, а в контекст
