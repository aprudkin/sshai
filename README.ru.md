...
`Windows PowerShell` — по SSH. При явно выбранном локальном выполнении она также запускает
локальный интерпретатор `Bash` или `PowerShell 7` (`pwsh`). Для удалённого `Windows` по умолчанию
предпочитается `PowerShell 7`; если он недоступен, используется `Windows PowerShell 5.1`, а для
отдельного запуска можно явно потребовать любую из этих оболочек. Полученный вывод сохраняется
локально, а в контекст агента возвращается компактный паспорт запуска.

Команды внутрь. Компактные доказательства наружу.
...
Пакет также содержит расширение Pi. После `pi install <share-root>` подключаются и навык агента, и
расширение. Режим сессии по умолчанию выключен и сохраняется в дереве сессии Pi:
...
Когда режим включён, Pi показывает `sshai:on` в нижней строке и один раз добавляет рекомендательную
инструкцию к каждому событию запуска агента. Он предпочитает `sshai` только для подходящих
ограниченных неинтерактивных команд; это не авторизация, не граница безопасности и не замена прямым
инструментам там, где они подходят.

### Другие агентские среды

Команды `/sshai ...` относятся к Pi и не переносятся в другие среды. В среде с поддержкой навыков
агента загрузите `skills/sshai/SKILL.md` и примените на время сессии такую инструкцию:

```text
Загрузи и соблюдай навык sshai. Предпочитай sshai local для потенциально шумных или повторяющихся
локальных неинтерактивных команд. Сохраняй полный вывод в артефакте, а нужные доказательства
получай через sshai q, sshai diff или --delta. Для коротких команд, файловых операций,
интерактивных и потоковых задач, а также неподдерживаемых сценариев используй прямые инструменты.
Эта рекомендация не предоставляет полномочий.
```

Для нативного переключателя реализуйте такой же по умолчанию выключенный флаг сессии через `API`
плагинов или хуков среды и добавляйте эту инструкцию перед каждым ходом агента. Сама интеграция не
должна запускать команды.

## Возможности

- Запуск на одном или нескольких SSH-алиасах `Linux` через `Bash` по умолчанию или через выбранную
  `POSIX`-оболочку.
- Запуск на алиасах `Windows`/`PowerShell` с предпочтительным `PowerShell 7`, автоматическим
  переключением на `Windows PowerShell 5.1` или явно заданной оболочкой.
- Запуск явно выбранного локального `bash` или `pwsh` через `sshai local`; это не запасной вариант
  SSH.
...
Для удалённого выполнения нужны OpenSSH и настроенный SSH-алиас, а для локального — `bash` или
`pwsh` в `PATH`. Для сборки из исходного кода также требуется Go `1.26.5` или новее в ветке `1.26`.

### Через Homebrew

Установите утилиту, вложенный навык агента и расширение Pi из репозитория `aprudkin/tap`:

```bash
brew install aprudkin/tap/sshai
```

В Pi эта команда подключает оба вложенных ресурса:

```bash
pi install "$(brew --prefix sshai)/share/sshai"
```

Другие среды с поддержкой навыков агента могут загрузить `skills/sshai/SKILL.md` из каталога пакета,
путь к которому выводит `brew --prefix sshai`. Вместо команд Pi используйте эквивалентную инструкцию
уровня сессии или нативный плагин либо хук.
...
Установщик помещает утилиту в `${SSHAI_INSTALL_DIR:-$HOME/.local/bin}`, а навык агента и расширение
Pi — в `${SSHAI_SHARE_DIR:-$HOME/.local/share/sshai}`. Pi может загрузить этот каталог как локальный
пакет:
...
настроенный каталог навыков и используйте эквивалентную инструкцию уровня сессии или нативный
плагин/хук.
...
Запускайте локальную команду только когда задача явно требует локального выполнения. Выберите
интерпретатор; `bash` и `pwsh` должны быть доступны через `PATH`:
...
Локальные результаты используют те же ограниченные артефакты, паспорта, JSON-конверт v1,
`--delta`, именованное состояние, историю, `q` и хранение, что и удалённые. Их синтетические цели
`local-bash` и `local-pwsh` видны в результатах и журнале, но не появляются в `sshai hosts`.
Локальный PowerShell запускает только `pwsh`; запасного варианта с `Windows PowerShell 5.1` нет.
...
На Windows оболочка по умолчанию предпочитает PowerShell 7 и переключается на Windows PowerShell
5.1, если PowerShell 7 недоступен. Чтобы потребовать PowerShell 7 без переключения, укажите
`--powershell-host pwsh`. Если команде нужен Windows PowerShell 5.1, выберите его явно:
...
stderr SSH не раскрывается. Если на хосте Windows ни одна из поддерживаемых форм настройки
PowerShell не создаёт требуемый рабочий каталог, sshai сохраняет `setup-error=windows-shell` с
фиксированной диагностикой, возвращает код `99`, не запускает тело пользователя и не кэширует
факты хоста. Явно разрешённое принятие ключа также возвращает его алгоритм и SHA-256-отпечаток.
...
- Отказ политики, транспортный сбой, сбой настройки оболочки Windows, локальный сбой и ненулевой
  код завершения удалённой команды — разные исходы.
- `sshai local` не является ни SSH, ни запасным вариантом удалённого выполнения, ни проверкой
  readonly-политики, ни слоем авторизации, ни песочницей безопасности. Он отклоняет удалённые
  параметры и `--follow`.
- Сбои запуска локального интерпретатора, тайм-ауты и ограничения вывода сохраняются как
  `local-error=start`, `local-error=timeout` и `local-error=output-limit`; каждый возвращает код
  процесса `96`. При тайм-ауте или переполнении вывода останавливается только прямой дочерний
  интерпретатор; очистка дочерних процессов на всех платформах не гарантируется.
