...
func Search(db *sql.DB, host, grep string, since time.Time, limit int) ([]artifact.Meta, error) {
	...
	query := `SELECT art_id, ts, host, ctx, command, exit, transport_error, bytes, lines, sha256, duration_ms, truncated, binary, delta_base FROM runs`
	if len(conds) > 0 {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &tsStr, &m.Host, &m.Ctx, &m.Command, &m.Exit, &m.TransportErr,
