...
extension="$share_dir/extensions/sshai-mode/index.ts"
skill="$share_dir/skills/sshai/SKILL.md"
cmp -s "$extension" "$repo_dir/extensions/sshai-mode/index.ts"
...
rm -- "$extension"
ln -s /dev/null "$extension"
if run_install >/dev/null 2>&1; then
  printf '%s\n' "installer accepted an extension symlink destination" >&2
  exit 1
fi
rm -- "$extension"
mkfifo "$extension"
if run_install >/dev/null 2>&1; then
  printf '%s\n' "installer accepted a non-regular extension destination" >&2
  exit 1
fi
